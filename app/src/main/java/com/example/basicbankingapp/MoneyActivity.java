package com.example.basicbankingapp;

import androidx.appcompat.app.AppCompatActivity;

import android.content.Intent;
import android.os.Bundle;
import android.view.View;
import android.widget.Button;
import android.widget.EditText;
import android.widget.Toast;

public class MoneyActivity extends AppCompatActivity {
    int idr;
    double amount,credit;
    DataBaseHelper mydb;
    private EditText etxtAmount;
    private Button btnContinue;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_money);
        etxtAmount = findViewById(R.id.etxt_amount_id);
        btnContinue = findViewById(R.id.btn_continue_id);
        Bundle bundle = getIntent().getExtras();
        idr = bundle.getInt("id");
        amount = bundle.getDouble("amount");

        btnContinue.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                if(checkCredit()) {
                    Intent intent = new Intent(MoneyActivity.this, SelectCustomerActivity.class);
                    intent.putExtra("id", idr);
                    intent.putExtra("amount", credit);
                    startActivity(intent);
                }
            }
        });
    }
    public boolean checkCredit(){

        credit = Double.parseDouble(etxtAmount.getText().toString());
        if(credit<=0.0) {
            etxtAmount.setError("Enter Amount");
            Toast.makeText(MoneyActivity.this,"Enter Amount more than 0",Toast.LENGTH_LONG).show();
            return false;
        }else {
            if(amount>=credit) {
                return true;
            }
            else{
                etxtAmount.setError("Change Amount");
                Toast.makeText(MoneyActivity.this,"Entered Amount Is Greater Than Maximum Available Amount",Toast.LENGTH_LONG).show();
                return false;
            }
        }
    }
}